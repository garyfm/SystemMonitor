...
public:
    ...
    int move_curser_to_next_field(WINDOW *win, int field_index, const int y_pos);
    int move_curser_to_previous_field(WINDOW *win, int field_index, const int y_pos);
    void print_header_info(const SystemMonitor& system_monitor);
    void print_process_info(WINDOW *process_info_w, Process& process, const int y_pos);
    void init(const SystemMonitor& System_monitor);
    void draw();
    void update_input_curser();
